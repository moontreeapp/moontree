...
class ImportedDerivedWallet extends DerivedWallet {
  ImportedDerivedWallet(seed, {net = Net.Test, cipher = const NoCipher()})
      : super(seed, net: net, cipher: cipher);

  factory ImportedDerivedWallet.fromEncryptedSeed(encryptedSeed,
      {name = 'Wallet', net = Net.Test, cipher = const NoCipher()}) {
    return ImportedDerivedWallet(cipher.decrypt(encryptedSeed),
        net: net, cipher: cipher);
  }

  factory ImportedDerivedWallet.fromRecord(records.ImportedDerivedWallet record,
      {cipher = const NoCipher()}) {
    return ImportedDerivedWallet(cipher.decrypt(record.encryptedSeed),
        net: record.net, cipher: cipher);
  }

  records.ImportedDerivedWallet toRecord() {
    return records.ImportedDerivedWallet(encryptedSeed, net: net);
  }
