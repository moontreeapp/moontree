...
extension AccountHasManyWallets on Account {
  List<Wallet> get wallets => res.wallets.byAccount.getAll(accountId);
...
extension AccountHasManyVouts on Account {
  Iterable<Vout> get vouts =>
      res.vouts.data.where((vout) => vout.account?.accountId == accountId);
}

extension AccountHasManyVins on Account {
  Iterable<Vin> get vins =>
      res.vins.data.where((vin) => vin.account?.accountId == accountId);
}
