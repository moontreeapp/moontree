...
class SubscriptionService {
  ...
  Future<void> setupClient(
    ...
    // Add a listener to monitor connectivity changes
    monitor.addListener((connected) {
      if (!isConnected) {
        see('Disconnected, attempting to reconnect...');
        _attemptReconnect(retryCount, retryDelay);
      }
    });

    await _setupConnection(retryCount, retryDelay);
  }

  Future<void> _setupConnection(int retryCount, Duration retryDelay) async {
  ...
  Future<void> _cancelListeners() async {
    ...
    listeners.clear(); // Clear the listeners list to avoid memory leaks
  }

  Future<void> _attemptReconnect(int retryCount, Duration retryDelay) async {
    for (int attempt = 1; attempt <= retryCount; attempt++) {
      try {
        await setupClient(monitor,
            retryCount: retryCount, retryDelay: retryDelay);
        see('Reconnected successfully on attempt $attempt');
        return;
      } catch (e) {
        see('Reconnect attempt $attempt failed: $e');
      }
      if (attempt < retryCount) {
        see('Retrying in ${retryDelay.inSeconds} seconds...');
        await Future.delayed(retryDelay);
    ...
    see('Failed to reconnect after $retryCount attempts');
  }
